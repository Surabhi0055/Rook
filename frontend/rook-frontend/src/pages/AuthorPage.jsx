...
function AuthorPhoto({ name }) {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(title)}&prop=pageimages&format=json&pithumbsize=200&origin=*`,
    )
      .then((r) => r.json())
      .then((d) => {
    ...
    >
      {books.map((b, i) => (
        <div
          ...
          onClick={() => onOpen(b)}
          onMouseEnter={(e) =>
            (e.currentTarget.querySelector(".gc-cover").style.transform =
...
export function AuthorPage({ onOpen, bookProps, initialAuthor }) {
  ...
  function loadAuthor(name) {
    ...
    setActiveGenre("All");
    apiFetch(
      `${API_BASE}/recommend/author?author=${encodeURIComponent(name)}&top_n=60`,
    )
  ...
  useEffect(() => {
    if (initialAuthor) loadAuthor(initialAuthor);
  }, [initialAuthor]);
  ...
  return (
    ...
    >
      <PageBanner
        ...
        sub="Find every book by your favourite authors"
      />
            ...
            >
              <div
                style={{
                  ...
                  transform:
                    selectedAuthor === a.name ? "scale(1.08)" : "scale(1)",
                }}
              >
      ...
      {selectedAuthor ? (
                ...
                >
                  {genreList.map((g) => (
                    <option
                      key={g}
                      value={g}
                      style={{ background: "#1a0608", color: "#f0e8dc" }}
                    >
                      {g}
                    </option>
                  ))}
        ...
        >
          Search for an author above or click a featured name to see their
          books.
        </div>
      )}
    </div>
